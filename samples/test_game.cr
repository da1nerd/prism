...
class TestGame < Prism::Game
  ...
  def init
    ...
    point_light = PointLight.new(Vector3f.new(0,1,0), 0.4, Vector3f.new(0,0,1), Vector3f.new(5, 0, 5), 100.0)
    point_light_object.add_component(point_light)

    spot_light_object = GameObject.new()
    spot_light = SpotLight.new(Vector3f.new(0,1,1), 0.4, Vector3f.new(0,0,0.1), Vector3f.new(8, 0, 8), 100.0, Vector3f.new(1, 0, 0), 0.7)
