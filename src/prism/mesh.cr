...
module Prism
  ...
  class Mesh
    ...
    def initialize(verticies : Array(Prism::Vertex), indicies : Array(LibGL::Int))
    ...
    private def load_mesh(file_name : String)
      ...
      verticies = [] of Prism::Vertex
      0.upto(model.positions.size - 1) do |i|
        verticies.push(Prism::Vertex.new(model.positions[i], model.tex_coords[i], model.normals[i]))
      end

      add_verticies(verticies, model.indicies, false)
    end

    def add_verticies(verticies : Array(Prism::Vertex), indicies : Array(LibGL::Int))
      add_verticies(verticies, indicies, false)
    end

    private def add_verticies(verticies : Array(Prism::Vertex), indicies : Array(LibGL::Int), calc_normals : Bool)
      ...
      LibGL.buffer_data(LibGL::ARRAY_BUFFER, verticies.size * Prism::Vertex::SIZE * sizeof(Float32), Prism::Vertex.flatten(verticies), LibGL::STATIC_DRAW)
      LibGL.bind_buffer(LibGL::ARRAY_BUFFER, 0)

      LibGL.bind_buffer(LibGL::ELEMENT_ARRAY_BUFFER, @resource.ibo)
      LibGL.buffer_data(LibGL::ELEMENT_ARRAY_BUFFER, indicies.size * Prism::Vertex::SIZE * sizeof(Float32), indicies, LibGL::STATIC_DRAW)
    ...
    def draw
      ...
      LibGL.vertex_attrib_pointer(0, 3, LibGL::FLOAT, LibGL::FALSE, Prism::Vertex::SIZE * sizeof(Float32), mesh_offset)

      texture_offset = Pointer(Void).new(3 * sizeof(Float32)) # TRICKY: skip the three floating point numbers above
      LibGL.vertex_attrib_pointer(1, 2, LibGL::FLOAT, LibGL::FALSE, Prism::Vertex::SIZE * sizeof(Float32), texture_offset)

      normals_offset = Pointer(Void).new(5 * sizeof(Float32)) # TRICKY: skip the five floating point numbers above
      LibGL.vertex_attrib_pointer(2, 3, LibGL::FLOAT, LibGL::FALSE, Prism::Vertex::SIZE * sizeof(Float32), normals_offset)
    ...
    private def calc_normals(verticies : Array(Prism::Vertex), indicies : Array(LibGL::Int))
      i = 0
